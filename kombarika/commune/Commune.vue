<template>
	...
	<div className="mb-3"> 
	 	...
	 	<select className="form-control" name="district" id="select-district">
			<option v-for='elt in district' :key='elt.id' :value='elt.id'>{{elt.nomDistrict}}</option>
        ...
        <div class="row">
			...
			<th> Id </th>
                ...
                <tbody>
                    <tr v-for="item in commune" :key="item.id">
...
<script>
	...
	district: null,
        ...
        methods: {
            async getAll() {
                /*try {
                    this.loading = true;
                    const response = await fetch(this.url + 'commune');
                    if (!response.ok) {
	...
	fetch(this.url + 'district') 
	 .then(response => response.json()) 
	.then(data => { this.district = data; }) 
	 .catch(error => alert(error)); 
