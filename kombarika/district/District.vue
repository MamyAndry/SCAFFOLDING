<template>
	...
	<div className="mb-3"> 
	 	...
	 	<select className="form-control" name="region" id="select-region">
			<option v-for='elt in region' :key='elt.id' :value='elt.id'>{{elt.nomRegion}}</option>
        ...
        <div class="row">
			...
			<th> Id </th>
                ...
                <tbody>
                    <tr v-for="item in district" :key="item.id">
	...
	region: null,
        ...
        methods: {
            async getAll() {
                /*try {
                    this.loading = true;
                    const response = await fetch(this.url + 'district');
                    if (!response.ok) {
